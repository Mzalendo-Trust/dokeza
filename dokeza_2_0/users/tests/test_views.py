...
from dokeza_2_0.users.tests.factories import UserFactory
from dokeza_2_0.users.views import (
    UserRedirectView,
    UserUpdateView,
    user_detail_view,
)
...
class TestAnnotationsView:
    def test_get_user_annotations(self, user: User, rf: RequestFactory):
        view = UserAnnotationView()
        request = rf.get("/fake-url")
        request.user = UserFactory()